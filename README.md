...
The service stores task events and reconstructs entities on request. Fetch assembled tasks from `/api/tasks` and post events (including user registration) to `/api/events`.
