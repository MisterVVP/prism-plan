...
internal sealed class LogoutUser(IUserEventRepository userRepo, IEventQueue eventQueue) : ICommandHandler<LogoutUserCommand>
{
    ...
    {
        var ev = new Event(Guid.NewGuid().ToString(), request.UserId, EntityTypes.User, UserEventTypes.Logout, null, DateTimeOffset.UtcNow.ToUnixTimeMilliseconds(), request.UserId);
